...
public class GuiBookContent {
	...
	for (int i = 0; i < contentList.size(); i++) {
		...
		List list = render.listFormattedStringToWidth(contentList
			.get(i).toString(), y);
		...
		render.drawSplitString((String) contentList.get(i), minX
			+ (genOffset * render.FONT_HEIGHT), maxX
			+ (genOffset * render.FONT_HEIGHT), 116, 0);

		genOffset += offset + 1;

	    } else if (contentList.get(i) instanceof Icon) {

		gui.drawTexturedModelRectFromIcon(minX, y + (genOffset * 16),
			(Icon) contentList.get(i), 16, 16);
