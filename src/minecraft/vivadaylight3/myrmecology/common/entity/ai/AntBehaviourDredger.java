...
public class AntBehaviourDredger extends EntityAIAntBehaviour {
	...
	if(block != null){
	    ...
	    ArrayList<ItemStack> stacks = Block.blocksList[block.ID].getBlockDropped(world, block.xCoord, block.yCoord, block.zCoord, block.metadata, 0);
	    
	    world.setBlockToAir(block.xCoord, block.yCoord, block.zCoord);
	    
	    for(ItemStack stack : stacks){
		
		Environment.spawnItem(stack, world, block.xCoord, block.yCoord, block.zCoord);
	...
	this.block = Environment.getNearestBlockFrom(blocks, (Entity) this.theAnt, getPosX(), getPosY(), getPosZ());
	
    }
    
    private int getTimeToWait() {
	return (int) (Block.blocksList[this.block.ID].getBlockHardness(world, block.xCoord, block.yCoord, block.zCoord) * 20);
    }
	...
	ArrayList<BlockPosEntry> temp = Environment.getBlocksInRadius(world, getPosX(), getPosY(), getPosZ(), 20);
	ItemStack tool = new ItemStack(Item.pickaxeStone);
	
	for(BlockPosEntry entry : temp){
	    
	    if(tool.canHarvestBlock(Block.blocksList[entry.ID]) && pathFinder.getPathToXYZ(entry.xCoord, entry.yCoord, entry.zCoord) != null && Block.blocksList[entry.ID].getBlockHardness(world, entry.xCoord, entry.yCoord, entry.yCoord) < maxHardness){
