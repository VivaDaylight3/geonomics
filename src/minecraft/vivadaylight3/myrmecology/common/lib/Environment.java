...
public class Environment {
    ...
    public static void addItemStackToInventory(ItemStack item, ItemStack[] inventory, int max, TileEntity tileEntity){
	...
	for(int k = 0; k < inventory.length; k++){
		...
		if (inventory[k] == null) {
			...
			inventory[k] = new ItemStack(item.getItem(), max, item.getItemDamage());
			
			tileEntity.onInventoryChanged();
			
		    }else if(amount < max){
			
			inventory[k] = new ItemStack(item.getItem(), amount, item.getItemDamage());
